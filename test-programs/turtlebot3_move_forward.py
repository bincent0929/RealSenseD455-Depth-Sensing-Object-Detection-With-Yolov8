...
class TurtleBot3MoveForward(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f"Target: Move forward {self.target_distance}m at {self.linear_speed}m/s"
        )
    ...
    def move_forward(self):
        ...
        if not self.odom_received:
            self.get_logger().warn(
                "No odometry received! Moving based on time instead."
            )
        ...
        while self.distance_traveled < self.target_distance:
            ...
            self.get_logger().info(
                f"Distance: {self.distance_traveled:.3f}m / {self.target_distance}m"
            )
        ...
        self.stop_robot()
        self.get_logger().info(
            f"Movement complete! Total distance: {self.distance_traveled:.3f}m"
        )
    ...
    def move_forward_timed(self):
        ...
        self.get_logger().info(
            f"Moving forward for {duration:.2f} seconds (time-based)"
        )
